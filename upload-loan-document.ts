...
export class UploadLoanDocumentComponent {
    ...
    uploadFile(event) {
        ...
        //if(files.type == "application/pdf" || files.type == "image/png")
        //{
        if (files && (files.type == "image/jpeg" || files.type == "application/pdf" || files.type == "image/png")) {
            if (files.size && files.size > 5000000) {
                ...
                this.dataCarrier.setCarrier('fileExtensionSecur', files.type);

                if (window['cordova']) {
                    var promise1 = this.readAsDataURL(files);
                ...
                this.dataCarrier.setCarrier("loanAgreeSecureFile", files.name);
        ...
        //    this.reader.readAsDataURL(fileToLoad);           
    ...
    onPreviewClick(){
       ...
       let linkSource =this.dataCarrier.getCarrier('secureBase64');
       let fileType = this.dataCarrier.getCarrier('fileExtensionSecur');
